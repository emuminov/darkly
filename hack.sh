...
step "1/13  IDOR: reading wil's private profile without any login"
run "idor/exploit.sh"

step "2/13  Open redirect: /redirect sends the victim anywhere"
run "open_redirect/exploit.sh"

step "3/13  Reflected XSS: the newsletter banner echoes raw HTML"
run "reflected_xss/exploit.sh"

step "4/13  No brute force protection: dictionary attack on jdoe"
run "no_brute_force_protection/exploit.sh"

step "5/13  Predictable password reset: token is md5(email)"
...
step "6/13  Broken access control: /api/grades leaks hidden flag and other students"
run "broken_access_control_api/exploit.sh"

step "7/13  Mass assignment: patching our own role"
run "mass_assignment/exploit.sh"

step "8/13  Stored XSS: stealing the moderation bot's cookie (may take up to 2 min)"
run "stored_xss/exploit.sh"

step "9/13  Path traversal: reading files outside the projects directory"
run "path_traversal/exploit.sh"

step "10/13  Unrestricted upload: avatar accepts any file type"
...
step "11/13  XXE to SSRF: making the server fetch /internal/config"
run "xxe_to_ssrf/exploit.sh"

step "12/13  Leaked JWT secret: forging an admin session token"
run "leaked_jwt_secret/exploit.sh"

step "13/13  PocketBase admin access: full database takeover"
